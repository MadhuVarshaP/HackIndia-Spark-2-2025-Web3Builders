...
const getWallet = (privateKey) => {
  ...
  const walletKey = privateKey || process.env.PRIVATE_KEY;
...
const createDeploymentLogger = (options) => {
  return (message) => {
    if (options.silent) {
      return;
    }
...
const resolveWallet = (options) => {
  return options?.wallet ?? getWallet();
};

const loadArtifact = async (deployer, contractArtifactName) => {
...
const deployTransparentProxy = async (
  contractArtifactName,
  constructorArguments,
  opts
) => {
...
const upgradeTransparentProxy = async (
  contractArtifactName,
  proxyAddress,
  opts
) => {
